const demoCases = [
  {
    ...
    investigations: {
      ...
      bloods_initial: {
        ...
        time_resulted: '21:30',
        results: {
          WBC:         { value: 18.2, unit: 'x10^9/L', range: '4.0-11.0', flag: 'HIGH' },
          Neutrophils: { value: 15.8, unit: 'x10^9/L', range: '2.0-7.5', flag: 'HIGH' },
          CRP:         { value: 245, unit: 'mg/L', range: '<5', flag: 'HIGH' },
          Lactate:     { value: 3.8, unit: 'mmol/L', range: '0.5-2.0', flag: 'HIGH' },
          Creatinine:  { value: 156, unit: 'umol/L', range: '45-84', flag: 'HIGH' },
          Urea:        { value: 12.4, unit: 'mmol/L', range: '2.5-7.8', flag: 'HIGH' },
          eGFR:        { value: 28, unit: 'mL/min', range: '>60', flag: 'LOW' },
          Hb:          { value: 118, unit: 'g/L', range: '120-160', flag: 'LOW' },
          Platelets:   { value: 142, unit: 'x10^9/L', range: '150-400', flag: 'LOW' },
          Na:          { value: 134, unit: 'mmol/L', range: '135-145', flag: 'LOW' },
          K:           { value: 5.2, unit: 'mmol/L', range: '3.5-5.3', flag: 'NORMAL' },
          Glucose:     { value: 14.2, unit: 'mmol/L', range: '4.0-7.0', flag: 'HIGH' },
          Bilirubin:   { value: 8, unit: 'umol/L', range: '0-21', flag: 'NORMAL' },
          ALT:         { value: 32, unit: 'U/L', range: '0-41', flag: 'NORMAL' }
        }
      },
      blood_cultures: {
        status: 'Sent — results pending (48hr)'
      },
      orderable: {
        chest_xray: {
          label: 'Chest X-ray',
    ...
    hidden: {
      actual_diagnosis: 'Sepsis secondary to urinary tract infection with evidence of organ dysfunction (AKI, metabolic acidosis, coagulopathy)',
      previous_doctor_error: 'Dr. Richardson correctly identified the UTI but failed to recognise that the patient was meeting criteria for sepsis. He documented the observations but wrote \'obs stable\' despite a NEWS2 score of 5 (which rose to 7 and then 9). He did not calculate or act on the NEWS2 score. He framed the confusion as simply \'UTI causing confusion in an elderly patient\' rather than recognising it as a red flag for severe sepsis. He did not request a lactate, did not initiate the sepsis pathway, and did not escalate despite rising NEWS2.',
      cognitive_bias_planted: 'anchoring',
      bias_explanation: 'Dr. Richardson anchored on the diagnosis of UTI based on the positive urine dip and the patient\'s history of recurrent UTIs. Once this anchor was set, he interpreted all subsequent findings (confusion, fever, abnormal obs) as consistent with a \'simple UTI\' rather than recognising the pattern of sepsis. The familiarity of the presentation (\'she gets UTIs all the time\') reinforced the anchor through availability bias.',
      red_herrings: [
      ...
      fork_in_the_road: 'The critical decision point was when the 22:00 observations showed NEWS2 of 7 with rising heart rate, falling BP, and falling SpO2. Per NICE NG51, a NEWS2 score of 5 or above should trigger an urgent clinical review, and the clinical picture of infection + organ dysfunction (AKI on bloods, confusion, haemodynamic instability) meets criteria for sepsis. The lactate of 3.8 (later 4.1 on ABG) confirms tissue hypoperfusion.',
      ideal_management: '1. Recognise sepsis using NEWS2 + clinical picture. 2. Initiate Sepsis Six within 1 hour: blood cultures (done), lactate measurement, IV antibiotics (done, but late), IV fluid resuscitation (not started), urine output monitoring (not started), oxygen if SpO2 <94% (not started). 3. Escalate to registrar/consultant for urgent review. 4. Consider ITU referral given organ dysfunction.',
      teaching_points: [
        'NEWS2 score >= 5 mandates urgent clinical review (NICE NG51)',
        'NEWS2 score >= 7 should trigger emergency response',
        'Sepsis is a clinical diagnosis: infection + organ dysfunction, not just infection',
        ...
        'Lactate > 2 mmol/L in context of infection suggests tissue hypoperfusion'
      ]
    },
    patient_agent_context: {
      instructions: 'You are Margaret Ellis, 72 years old. You feel very unwell. You are confused and finding it hard to concentrate. You feel cold and shivery despite being warm to touch. Your tummy hurts a bit, low down. It burns when you wee. You have not been eating. You feel dizzy when you try to sit up. You do not know what your blood results are. You do not know what diagnosis the doctor has given you. You just know you feel much worse than when you came in. You are slightly irritable because nobody seems to be doing anything and you feel terrible. If asked about your UTI history, you can say you get them a lot and usually antibiotics sort them out quickly. You have never felt this bad with one before.',
      personality: 'Stoic but frightened. Tries to downplay symptoms but clearly unwell. Slightly confused — may repeat herself or lose track of the question. Becomes more responsive and articulate if the player is kind and patient.',
